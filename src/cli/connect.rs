...
pub struct ConnectOpts {
    #[arg(value_parser = verify_conn_str, help = "Connection string to the dataset, could be postgres of local file (support: cvs, parquet, json)")]
...
fn verify_conn_str(s: &str) -> Result<DatasetConn, String> {
    ...
    } else if conn_str.starts_with("csv://") {
        Ok(DatasetConn::Csv(conn_str))
    } else if conn_str.starts_with(".parquet") {
        Ok(DatasetConn::Parquet(conn_str))
    } else if conn_str.starts_with(".json") {
